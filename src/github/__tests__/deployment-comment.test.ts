...
describe("deployment comments", () => {
  ...
  test("matches only an exact marker line for the requested stage", () => {
    ...
    expect(hasDeploymentCommentMarker("<!-- alchemy-deploy:pr-42 -->", stage)).toBeTrue();
    expect(hasDeploymentCommentMarker("prefix <!-- alchemy-deploy:pr-42 -->", stage)).toBeFalse();
    expect(hasDeploymentCommentMarker("<!-- alchemy-deploy:pr-420 -->", stage)).toBeFalse();
  });
});
